...
Uso:
    ...
    python batch_train.py configs/ae/*.yaml --skip-emb       # sin UMAP/t-SNE
"""
...
from componente_a.config import CULTIVOS, PANEL_PATH, RUNS_DIR, WANDB_ENTITY, WANDB_PROJECT, ExperimentConfig
...
def _run_single(task: Tuple[str, str, bool]) -> Dict:
    ...
    store = get_store(backend, runs_dir=RUNS_DIR, project=WANDB_PROJECT, entity=WANDB_ENTITY)

    ds = cdata.build_crop_dataset(panel_z, cultivo, exp_cfg)
    emb_methods = [] if skip_emb else ("umap", "tsne")
    ...
    result = run_model_multiseed(
        ...
        n_seeds=n_seeds, base_seed=base_seed, emb_methods=emb_methods,
    )
    store.save(result)
...
def main() -> None:
    ...
    ap.add_argument("--skip-emb", action="store_true",
                    help="Omite UMAP/t-SNE (más rápido, pero la app no muestra proyecciones)")
    ...
    if args.workers <= 1:
        for cfg_path, cultivo, skip_emb in tasks:
            print(f"\n{'─'*55}")
            print(f"Config: {cfg_path}  cultivo: {cultivo}")
            print(f"{'─'*55}")
            try:
                row = _run_single((cfg_path, cultivo, skip_emb))
        ...
        with ProcessPoolExecutor(max_workers=args.workers) as ex:
            ...
            for future in as_completed(future_to_task):
                cfg_path, cultivo, _ = future_to_task[future]
                done += 1
                try:
                    row = future.result()
                    all_results.append(row)
