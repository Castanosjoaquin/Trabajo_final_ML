...
Carga los runs ya guardados en runs/ (no re-entrena nada; siempre la corrida más
reciente de cada config) y expone tablas y plots. Los notebooks importan esto y
quedan finos: narrativa + tabla + gráfico.
"""
...
def compare_table(names_labels, cultivo, split="test") -> pd.DataFrame:
...
def summary_fields(names_labels, cultivo, keys) -> pd.DataFrame:
    """Campos crudos del summary de varios runs (p. ej. recall estratificado)."""
    rows = []
    for name, label in names_labels:
        rid = latest_run(name, cultivo)
        if not rid:
            continue
        s = load_run(rid).summary
        rows.append({"modelo": label, **{k: s.get(f"test_{k}", s.get(k)) for k in keys}})
    return pd.DataFrame(rows)
...
def describe_architecture(config, n_features=54):
    ...
    if any(config.get(k) is not None for k in ("lr", "max_epochs", "patience", "batch_size")):
...
# Plots
# ===========================================================================
def plot_compare(names_labels, cultivo, metric="pr_auc", baseline=None, title=None):
...
def plot_score_hist(name, cultivo, bins=40, ax=None):
...
def plot_score_hist_grid(names_labels, cultivo, bins=40, ncols=2):
    ...
    for k, (name, label) in enumerate(names_labels):
        plot_score_hist(name, cultivo, bins=bins, ax=axf[k])
...
def plot_embeddings(name, cultivo, method="umap", color_by="label", ax=None):
...
def _labels_preds(name, cultivo, split="test", op=None):
...
def plot_confusion_matrix(name, cultivo, split="test", normalize=False, ax=None, op=None):
    ...
    y, yp = _labels_preds(name, cultivo, split, op=op)
    if y is None:
        print(f"No hay run para {name} ({cultivo})"); return None
...
def confusion_report(name, cultivo, split="test", ops=(0.10, "base")) -> pd.DataFrame:
...
# ===========================================================================
# Tablas de resultados VALIDADAS (multi-seed) — fuente única
# ===========================================================================
def tbl_leaderboard() -> pd.DataFrame:
    """Leaderboard final. PR-AUC media±std, test."""
    return pd.DataFrame([
