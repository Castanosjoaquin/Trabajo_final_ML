...
Lee los run-dirs locales (carpeta `runs/`). Ejecutar:
...
def _list_runs(runs_dir):
    return ResultsStore(runs_dir).list_runs()


@st.cache_data(show_spinner=False, ttl=300)
def _load_run(runs_dir, run_id):
    r = ResultsStore(runs_dir).load_run(run_id)
    # Las proyecciones 2D no se calculan al entrenar: se computan on-demand la
    # primera vez que se ve el run y se cachean en el run-dir (ver embeddings.py).
    emb = r.embeddings
    if emb is None or emb.empty:
        try:
            from src.embeddings import compute_embeddings
            emb = compute_embeddings(run_id, runs_dir=runs_dir)
        except Exception:
            emb = r.embeddings   # queda vacío → la app muestra "sin proyección"
...
# -----------------------------------------------------------------------------
# Sidebar — cultivo, selección de runs
# -----------------------------------------------------------------------------
...
try:
    ...
    st.error(f"No se pudo listar runs: {e}")
    st.stop()
...
run_a = _load_run(runs_dir, labels[sel_a])
run_b = _load_run(runs_dir, labels[sel_b])
